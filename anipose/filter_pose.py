...
def filter_pose_viterbi(config, all_points, bodyparts):
    ...
    points_full[scores_full < config['score_threshold']] = np.nan
    ...
    max_offset = config['n_back']
    thres_dist = config['offset_threshold']
...
def filter_pose_medfilt(config, all_points, bodyparts):
    ...
    for bp_ix, bp in enumerate(bodyparts):
        ...
        xmed = signal.medfilt(x, kernel_size=config['medfilt'])
        ymed = signal.medfilt(y, kernel_size=config['medfilt'])
        ...
        bad[err >= config['offset_threshold']] = True
        bad[score < config['score_threshold']] = True
...
def process_peter(config, pose_folder, out_folder):
    ...
    for fname in pose_files:
        all_points, metadata = tools.load_pose_2d(fname)
        for filter_type in filter_types:
        ...
        out_pn = os.path.join(out_folder, basename)
        tools.write_pose_2d(all_points[:,:,0,:], metadata, out_pn)
